...
# def drn_d_54_seg(num_classes, pretrained=True):
#     base_model = drn_d_54(pretrained=pretrained, out_map=False,out_middle=True)
#     base_model.fc = nn.Conv2d(512, num_classes, kernel_size=1,
#                               stride=1, padding=0, bias=True)
#     return base_model

class drn_d_54_seg(nn.Module):
    def __init__(self, num_classes=2,seg_class=1, pretrained=True):
        super(drn_d_54_seg, self).__init__()
        self.base_model = drn_d_54(pretrained=pretrained, out_map=False, out_middle=True)
        self.base_model.fc = nn.Conv2d(512, num_classes, kernel_size=1,
                                  stride=1, padding=0, bias=True)
        self.seg_fc= nn.Conv2d(512, seg_class, kernel_size=1,
                                  stride=1, padding=0, bias=True)
    def forward(self, x):
        cls, segmap =self.base_model(x)
        segout= self.seg_fc(segmap)
        return cls,F.upsample_bilinear(segout,scale_factor=8)

if __name__ == '__main__':
    import numpy as np
    model = drn_d_54_seg(num_classes=2,pretrained=True)
    print(model)
    x = Variable(torch.from_numpy(np.empty((5,3,384,512))).float())
    cls,segmap= model(x)
    print(cls.size(),segmap.size())