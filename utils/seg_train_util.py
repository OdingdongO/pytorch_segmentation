    ...
    for epoch in range(start_epoch,epoch_num):
        ...
        for batch_cnt, data in enumerate(data_loader['train']):
            ...
            if step % val_inter == 0:
                ...
                t0 = time.time()
                mIOU,val_acc = predict(model, data_set['val'], data_loader['val'],num_classes=4, counting=False)
                t1 = time.time()
                since = t1-t0
            ...
            iou = cal_IOU(preds.data.cpu().numpy(), masks.cpu().data.numpy(), num_classes=4)
