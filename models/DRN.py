...
def drn_d_54_seg(num_classes, pretrained=True):
    base_model = drn_d_54(pretrained=pretrained, out_map=True)
...
if __name__ == '__main__':
    import numpy as np
    model = drn_d_54_seg(num_classes=24,pretrained=True)
    x = Variable(torch.from_numpy(np.empty((5,3,512,512))).float())
    print model(x).size()