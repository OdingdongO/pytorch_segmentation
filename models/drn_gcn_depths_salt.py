...
class DRN_GCN_Depth(nn.Module):
    def __init__(self, num_classes, layers=50,img_gray=False):
        super(DRN_GCN_Depth, self).__init__()
        ...
        self.gcm1 = _GlobalConvModule(513, num_classes, (7, 7))
    ...
    def forward(self, x,depth):
        ...
        H,W = x.size()[2:]
        # if x: 128
        fm0 = self.layer0(x)  # bs, 32L, 64L, 64L
        # print(fm0.size())
        fm1 = self.layer1(fm0)  # bs ,256L, 32L, 32L
        # print(fm1.size())
        fm2 = self.layer2(fm1)  # bs, 512L, 16L, 16L
        # print(fm2.size())
        fm3 = self.layer3(fm2)  # bs,2048L, 16L, 16L
        # print(fm3.size())
        fm4 = self.layer4(fm3)  # bs,512L, 16L, 16L
        # print(fm4.size())

        depth=depth.repeat(1, 256).view(depth.size(0),1,16,16)
        fm4 = torch.cat([fm4,depth],dim=1)
        # print(fm4.size())
...
if __name__ =='__main__':
    model = DRN_GCN_Depth(2,  layers=50)
    # print model
    x = torch.FloatTensor(2,3,128,128)
    depth = torch.FloatTensor(2,1)
